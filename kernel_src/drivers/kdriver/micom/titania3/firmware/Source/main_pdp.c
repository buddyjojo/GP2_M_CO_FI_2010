...
#define PWM_DUTY_MAX			50//100	
#define PWM_DUTY_MIN			0//0//20	
...
static volatile U8  xdata gu8DutyRate = PWM_DUTY_MIN;//50;	// duty ratio value
static volatile BIT xdata guBDirect = TRUE;		// duty ratio up or down 

static volatile U8 	xdata gu81msCnt = 0;
static volatile U8 	xdata gu81msFlag = 0;

static volatile U8 	xdata gu850msFlag = 0;
static volatile U16 xdata gu1640msCnt = 0;

static volatile U8 	xdata gu8100msFlag = 0;
static volatile U16 xdata gu16100msCnt = 0;

static volatile U8 	xdata gu8LEDStep = 0;
static volatile U8 	xdata gu8LEDPosition = 0;

static volatile U8 	xdata gu8LEDGo = FALSE;
static volatile U8  xdata gu8DemoLEDON = FALSE;
...
void MDrv_PM_LED_ByLastPowerState(void)
{
    switch( gu8EepromData1 & 0x0F )
    {
        ...
        default:
            ...
            GPIO_PM0_OUTPUT_HIGH();		// for mercury
...
#else  //!(MS_BOARD_TYPE_SEL == BD_S6_PDP)

	GPIO_PM0_OUTPUT_LOW();	//LED_W1
	GPIO_PM1_INPUT();		//DBG_TX
	GPIO_PM2_OUTPUT_HIGH();	//5V_ON
...
void MDrv_PM_RunFrontLED(void)
{
   ...
   if (gu8LEDGo == TRUE)
   {
	  ...
	  if(gu8100msFlag)
	  {
		 ...
		 else if(gu8LEDStep == 25)
		 {
			gu8PWMClkCnt = 0xff;	//Stop LED on/off 
			LED_W2_HIGH();
		 }
		 else if(gu8LEDStep == 45)//30)
		 {	
			guBDirect = FALSE;
			gu8DutyRate = PWM_DUTY_MAX;
			gu8PWMClkCnt = 0;
		 }
		 else if(gu8LEDStep == 65)//50)
		 {
			gu8LEDPosition = 0;
			gu8PWMClkCnt = 0;
			guBDirect = TRUE;
		 ...
		 else if(gu8LEDStep == 105)//90)//80)
		 {
			guBDirect = FALSE;
			gu8DutyRate = PWM_DUTY_MAX;
			gu8PWMClkCnt = 0;
		 }
		 else if(gu8LEDStep == 125)//110)//100)
		 {
			gu8LEDGo = FALSE;
			gu8PWMClkCnt = 0xff;
			LED_W1_LOW();
...
//earnest add for switching Large Capital 
			if ( S0BUF >= 'A' || S0BUF <= 'Z' )
            	gu8Uart0CmdBuf[gu8Uart0BufIndex] = S0BUF + 'a';
			else
...
#if 0	//earenst
// conver PowerOffStatus from cpu notation to rs 232 notation
U8 Mdrv_PM_UART_Get_PowerOffStat(void)
{
   U8 data retVal = 0x0a;

   switch (gu8PowerOffMode)
   {
		case 0x10: 	// remote key
		   retVal = 0x02;
		   break;

		case 0x12: 	// sleep timer
		   retVal = 0x03;
		   break;
...
bool MDrv_PM_UART_ParseData(void)
{
   ...
   if(Data_Ready)
   {
	  ...
	  if((MakeLowerCase(TVLINK_CMD1) == 'k') && (MakeLowerCase(TVLINK_CMD2) == 'a'))
		...
		else if( (TVLINK_CMD1 == 'k') && (TVLINK_CMD2 == 'z') )
        {
            if(((gu8SetID == Mdrv_Get_TVLinkID()) || (Mdrv_Get_TVLinkID() == 0)) && (Mdrv_Get_TVLinkData0() == 0xff))
            {
                CMD_Ready = TRUE;
				CMD_KZ = TRUE;
            }
   ...
   //gu8Uart0BufIndex = 0;
    ...
    if(/*Mdrv_Get_TVLinkID() &&*/ gu8Uart0BufIndex > 0)
    {
        ...
        if(CMD_Ready)
        {
		   ...
		   if ( CMD_KZ )
		   {
			  ucBuf[0] = ConvertHexVal2Str((Mdrv_PM_UART_Get_PowerOffStat()>>4)&0x0F);
			  ucBuf[1] = ConvertHexVal2Str(Mdrv_PM_UART_Get_PowerOffStat()&0x0F);
		   }
#endif
...
void MDrv_PM_M4Sleep(U8 u8SleepMode, U16 U16SleepSource) small
{
   ...
   //#ifdef ENABLE_AC_DET   //20080807 for AC Detection
#ifdef LGE_USE_EEPROM
   // if( TRUE == gu8EepromData3) //TRUE: LCD or PDP
   {
	  SB_MUTE_ON();
   }
#endif
...
INT_SERVICE( EX0 )
{
    ...
    //                    gOTA_OnTimeInfo.EnaFlg  = EN_TIME_ONTIME_ONCE;   //not equal to zero when set OTA_ONTIMER
                    ...
                    case CP_WRITE_POWER_CONTROL:
                        ...
                        if( M_POWER_STATE_COLD_STANDBY !=gu32Received0.u8Num[3])
                        {
							if ( (gu8PowerOnOffMode != gu32Received0.u8Num[3])
					...
					case CP_WRITE_DISPLAY_MODE:		// Loging LED for demode
                        gu8DemoLEDON = gu32Received0.u8Num[3];
						break;
                    case CP_NO_CMD:
                    default:
...
//#ifdef ENABLE_AC_DET   //20080807 for AC Detection
#ifdef LGE_USE_EEPROM
//            if( TRUE == gu8EepromData3) //TRUE: LCD and  PDP
            {
                if( !AC_IS_DET() )
                {
                    SB_MUTE_OFF();
...
#if 1   //comment out for reducing code size purpose
