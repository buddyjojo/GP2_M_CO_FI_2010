...
//dhjung LGE
int lginit_mountflags = MS_RDONLY | MS_SILENT;
char * __initdata lginit_device_name;
static char __initdata saved_lginit_name[64];
...
{
	strlcpy(saved_lginit_name, line, sizeof(saved_lginit_name));
	return 1;
}
...
void __init mount_lginit(void)
{
	...
	if (saved_lginit_name[0]) {
		lginit_device_name = saved_lginit_name;
	}

	err = sys_mount(lginit_device_name, "/mnt/lg/lginit", "squashfs", lginit_mountflags, NULL);
	if (err)
	{
	...
	sys_chdir("/mnt/lg/lginit");
	printk("VFS: Mounted lginit (%s filesystem)%s.\n",
	       current->fs->pwd.mnt->mnt_sb->s_type->name,
	       current->fs->pwd.mnt->mnt_sb->s_flags & MS_RDONLY ?
	       " readonly" : "");
}
