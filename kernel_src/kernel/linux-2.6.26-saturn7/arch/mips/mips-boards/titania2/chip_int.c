...
void Titania_hw0_fiqdispatch(void)
{
    ...
    {
        do_IRQ((unsigned int)E_FIQ_INT_AEON_TO_8051);
    }
    if ( u16Reg & FIQL_8051_TO_AEON )
    {
        do_IRQ((unsigned int)E_FIQ_INT_8051_TO_AEON);
    }
    if ( u16Reg & FIQL_8051_TO_MIPS )
    {
        do_IRQ((unsigned int)E_FIQ_INT_8051_TO_MIPS);
    }
    if ( u16Reg & FIQL_MIPS_TO_8051 )
    {
        do_IRQ((unsigned int)E_FIQ_INT_MIPS_TO_8051);
    }
    if ( u16Reg & FIQL_MIPS_TO_AEON )
    {
        do_IRQ((unsigned int)E_FIQ_INT_MIPS_TO_AEON);
    }
    if ( u16Reg & FIQL_AEON_TO_MIPS )
    {
        do_IRQ((unsigned int)E_FIQ_INT_AEON_TO_MIPS);
    }
