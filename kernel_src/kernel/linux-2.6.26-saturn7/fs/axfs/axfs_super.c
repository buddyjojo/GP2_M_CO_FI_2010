...
static int axfs_check_page_shift(struct axfs_super *sbi)
{
	if (sbi->page_shift != PAGE_SHIFT) {
		printk(KERN_ERR "axfs: Filesystem is AXFS, however "
				...
				"of the system. (axfs=%d, system=%d) Cowardly refusing "
...
static int axfs_get_onmedia_super(struct super_block *sb)
{
	...
	printk("sbo base      = %p\n", sbo);
	printk("magic         = %d\n", (unsigned int)sbo - (unsigned int) &sbo->magic);
	printk("signature     = %d\n", (unsigned int)sbo - (unsigned int) &sbo->signature[0]);
	printk("digest        = %d\n", (unsigned int)sbo - (unsigned int) &sbo->digest[0]);
	printk("cblock_size   = %d\n", (unsigned int)sbo - (unsigned int) &sbo->cblock_size);
	printk("files         = %d\n", (unsigned int)sbo - (unsigned int) &sbo->files);
	printk("size          = %d\n", (unsigned int)sbo - (unsigned int) &sbo->size);
	printk("uids          = %d\n", (unsigned int)sbo - (unsigned int) &sbo->uids);
	printk("gids          = %d\n", (unsigned int)sbo - (unsigned int) &sbo->gids);
	printk("version_major = %d\n", (unsigned int)sbo - (unsigned int) &sbo->version_major);
	printk("version_minor = %d\n", (unsigned int)sbo - (unsigned int) &sbo->version_minor);
	printk("version_sub   = %d\n", (unsigned int)sbo - (unsigned int) &sbo->version_sub);
	printk("compression   = %d\n", (unsigned int)sbo - (unsigned int) &sbo->compression_type);
	printk("timestamp     = %d\n", (unsigned int)sbo - (unsigned int) &sbo->timestamp);
	printk("page_shift    = %d\n", (unsigned int)sbo - (unsigned int) &sbo->page_shift);
