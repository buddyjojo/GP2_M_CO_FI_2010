...
int fat_count_free_clusters(struct super_block *sb)
{
	...
	while (fatent.entry < sbi->max_cluster) {
		...
		#if 1
		if (ops->ent_get(&fatent) != FAT_ENT_FREE) {
			fatent.entry += (b_size - 1);
			fatent.u.ent32_p += (b_size - 1);
			if (ops->ent_get(&fatent) == FAT_ENT_FREE) {
				fatent.entry -= (b_size - 1);
				fatent.u.ent32_p -= (b_size - 1);
				do {
			...
			else //if (ops->ent_get(&fatent) != FAT_ENT_FREE)
			{
				fatent.entry += 1;
				fatent.u.ent32_p = NULL;
				continue;
		...
		if (ops->ent_get(&fatent) == FAT_ENT_FREE)
		#endif
			free += ((sbi->max_cluster - fatent.entry) < b_size) ? (sbi->max_cluster - fatent.entry) : b_size;
		fatent.entry += ((sbi->max_cluster - fatent.entry) < b_size) ? (sbi->max_cluster - fatent.entry) : b_size;
		fatent.u.ent32_p = NULL;
	#endif
	}
