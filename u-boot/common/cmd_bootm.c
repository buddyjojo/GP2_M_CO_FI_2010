...
int	lzo_do_decomp (unsigned char *, unsigned char *, unsigned long *);
...
int do_lzo (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	ulong	addr;
	ulong	data, len;
	...
	printf ("   Uncompressing from %x to %x... ", (ulong)data, (ulong)ntohl(hdr->ih_load));
	if (lzo_do_decomp ((uchar *)ntohl(hdr->ih_load), (uchar *) data, &len) != 0) {
		printf ("LZOGUNZIP ERROR - must RESET board to recover\n");
...
int do_bootm (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	{
		if(serial_tstc())
			key = (char) serial_getc();
		if (key == 0xd || key == 0x1b)
		{
			printf("-------------------------------------\n");
			printf(" Fast-boot interrupted. (key=0x%x)\n", key);
			printf(" Stop kernel booting.\n");
			printf("-------------------------------------\n");
			return 1;
		}
	}
	...
	printf ("[%ld] ## Booting image at %08lx ...\n", get_cur_time(), addr);
	...
	if (verify) {
		...
		if (crc32 (0, (uchar *)data, len) != ntohl(hdr->ih_dcrc)) {
			printf ("Bad Data CRC\n");
			printf ("\n   ERROR :: Failed to veeifying. Bad Data CRC.\n");
			printf ("\n   STOP booting.\n");
			SHOW_BOOT_PROGRESS (-3);
			return 1;
		}
