...
void usb_hub_reset_port(void); // ieeum
int Usb_Hub_Port_Num(struct usb_device *dev);
int USB_Hub_Handle(struct usb_device *dev, int port);
...
#endif
#define USB_MSEC_LOOP		( CPU_CLOCK_FREQ/10000/4 )
...
#define NEW_INIT_SEQ
	...
	//usb_hub_probe(dev,0);
	if (dev->config.if_desc[0].bInterfaceClass != 9) {
		if (UsbPortSelect==0)
...
int usb_scan_devices(void)
{
	...
	if (dev->config.if_desc[0].bInterfaceClass == 9) { //USB hub
		//dhjung LGE
		//printf("USB hub found\n");
		PortNum = Usb_Hub_Port_Num(dev);

		for (i=1; i <=PortNum ; i++) {
			if (USB_Hub_Handle(dev, i)) {
				dev->devnum = dev_index + 1;
				//dhjung LGE
				//printf("dev->devnum: %d\n", dev->devnum);
				dev_index++;
				NowIsHub = 1;
	...
	if (dev->config.if_desc[0].bInterfaceClass == 8) { //mass storage
		//do about mass storage
	} else {

		if (!usb_lan_initialize(dev))
			return -1;
	}
...
void usb_hub_reset_port(void)
{
	//Set SAR4(GPIO35)
	MHal_GPIO_REG(0x1423) |= BIT4;

	//Set OEN Out(GPIO35)
	MHal_GPIO_REG(0x1423)  &= ~BIT4;

	//Set Output High(GPIO35)
	MHal_GPIO_REG(0x1424)  |= BIT4;	
	mdelay(1);
	
	//Set Output Low(GPIO35)
	MHal_GPIO_REG(0x1424)  &= ~BIT4; 
	mdelay(1);
...
int Usb_Hub_Port_Num(struct usb_device *dev)
{
	...
	unsigned char PortNum, i;

	if (usb_get_hub_descriptor(dev, buffer, 9) < 0) {
		printf("Usb_Hub_Port_Num: failed to get hub descriptor, giving up %lX\n",dev->status);
		return -1;
	}
	...
	for (i=1; i<=PortNum; i++)
	{
		if (usb_set_port_feature(dev, i, 0x08) < 0)
		{
			printf("Usb_Hub_Port_Num: failed to set port feature\n");
			return PortNum;
		}
	}
	...
	for(i=1;i<=PortNum;i++)
	{
		...
		if (usb_clear_port_feature(dev, i, 0x10) < 0)
		{
			printf("Usb_Hub_Port_Num: failed to clear port feature\n");
			return PortNum;
		}
	}
...
int USB_Hub_Handle(struct usb_device *dev, int port)
{
	...
	if (usb_get_port_status(dev, port, buffer) < 0)
	{
		printf("USB_Hub_Handle: failed to usb_get_port_status1\n");
		return 0;
	}

	//dhjung LGE
	//printf("Port:%d Status=%02x\n",port, (*buffer));

	if(!((*buffer)&0x01))
		return 0;

	//printf("Port %d have device.\n",port);

	if (usb_set_port_feature(dev, port, 0x04) < 0)		//SET_FEATURE_PORT_RESET
	{
		printf("USB_Hub_Handle: failed to SET_FEATURE_PORT_RESET\n");
		return 0;
	}
	...
	if (usb_get_port_status(dev, port, buffer) < 0)
	{
		printf("USB_Hub_Handle: failed to usb_get_port_status2\n");
		return 0;
	}

	//clear feature c_port_reset
	if (usb_clear_port_feature(dev, port, 0x14) < 0)	//CLEAR_FEATURE_C_PORT_RESET
	{
		printf("USB_Hub_Handle: failed to usb_clear_port_feature\n");
		return 0;
	}

	if (usb_get_port_status(dev, port, buffer) < 0)
	{
		printf("USB_Hub_Handle: failed to usb_get_port_status3\n");
		return 0;
	}
