...
{
	struct mtd_partition_info *mpi = NULL, *unmpi = NULL, *mtdmpi = NULL;;
	...
	if(!strcmp(mpi->name, "mtdinfo")) {
		pmi = (struct boot_mtd_info *)(offset);
		do {
...
#else
		ret = write_flash_data(&nand_info[0], (ulong)floffset, (ulong)size, (u_char *)offset);
		if(ret) {
...
_epk_start:
	...
	GET_MTD_INFO()->old_epk_ver = GET_MTD_INFO()->cur_epk_ver;
	GET_MTD_INFO()->cur_epk_ver = epk->epkVersion;
