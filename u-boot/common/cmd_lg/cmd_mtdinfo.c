...
unsigned int load_default_mtdinfo(int bsave);
...
unsigned int config_mtdinfo(void)
{
	...
	do {
		...
		switch(rchr) {
			...
			case 'd' : case 'D' : 	load_default_mtdinfo(1);	break;
...
unsigned int save_mtdinfo(void)
{
	...
	dprintf("\n%s: magic [%08x] nmap=[%d] npartition=[%d]\n", __func__,
			mtdinfo.magic, mtdinfo.nmap, mtdinfo.npartition);
#endif
...
unsigned int load_mtdinfo(void)
{
	...
	size = sizeof(struct boot_mtd_info);
	ret = nand_read(&nand_info[0], (ulong)DEFAULT_MTDINFO_BASE, &size, (u_char *)pmi);
	if(ret) {
