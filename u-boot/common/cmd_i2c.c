...
//static uchar	i2c_mm_last_channel;
...
//static int mod_i2c_mem(cmd_tbl_t *cmdtp, int incrflag, int flag, int argc, char *argv[]);
...
int do_i2c_read ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	addr[0] = simple_strtoul(argv[3], NULL, 16);
    addr[1] = simple_strtoul(argv[4], NULL, 16);
    alen = simple_strtoul(argv[5], NULL, 16);;
    ...
    i2c_read (channel,chip, (uint*)addr, alen, buffer, len);
    //i2c_read (channel,chip, addr, alen, buffer, len);
...
int do_i2c_read_byte ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    //for EEPROM(AT24C512), if addr is one byte, this function must be modified.
	regoffset[0] = 0;
	regoffset[1] = simple_strtoul(argv[3], NULL, 16);

    i2c_read (channel,devaddr, (uint*)regoffset, 2, &value, 1);
    printf("value=%d\n", value);
    return 0;
}
...
int do_i2c_write_byte ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	subaddrmode = simple_strtoul(argv[5], NULL, 10);

	printf("channel = %d \n",channel);
	printf("devaddr = 0x%x \n",devaddr);
	printf("subaddr mode = %d",subaddrmode);
	printf("regoffset = %d \n",regoffset[1]);
	printf("value = %d \n",value);
	if(subaddrmode == 0)
		i2c_write (channel,devaddr, regoffset, 0, &value, 1);
	else
    	i2c_write (channel,devaddr, regoffset, 2, &value, 1);
	return 0;
}

#if 0  //redcloud00@lge.com
...
int do_sdram  ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if(i2c_read(channel, chip, 0, 1, data, sizeof(data)) != 0) {
...
U_BOOT_CMD(
	i2c_r,	7,	1,	do_i2c_read,
	"i2c_r   - i2c read \n",
	"command: i2c_r channel slaveID(hex) addr[0](hex) addr[1](hex) alen(hex) len(hex)\n"
);

U_BOOT_CMD(
	i2c_rb,	4,	1,	do_i2c_read_byte,
	"i2c_rb  - i2c read for one byte\n",
	"command: i2c_rb channel slaveID(hex) offset(hex)\n"
);

U_BOOT_CMD(
	i2c_wb,	6,	1,	do_i2c_write_byte,
	"i2c_wb  - i2c write for one byte\n",
	"command: i2c_wb channel slaveID(hex) offset(hex) value(hex) subaddrmode\n"
);
