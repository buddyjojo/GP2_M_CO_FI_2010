...
static void Usage(void)
{
	char *sUsage =
			...
			"  -- Jackee, Lee, jackee@lge.com(build 2007/11/30)"
			"  ";
...
int main(int argc, char **argv)
{
	...
	for(i = 1; i < argc; i++)
	{
		...
		else if (i == cmd_index)
		{
			...
			else if (strcmp(crossBase, "ld" ) == 0) strcpy(gAction, "Linkng");
			else if (strcmp(crossBase, "cc" ) == 0) strcpy(gAction, "Making");
			else if (strcmp(crossBase, "gcc") == 0) strcpy(gAction, "Making");
			else if (strcmp(crossBase, "g++") == 0) strcpy(gAction, "Making");
		}
