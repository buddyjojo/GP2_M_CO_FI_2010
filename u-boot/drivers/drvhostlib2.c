...
UINT8 QtdBufP1[Host20_qTD_SIZE*Host20_qTD_MAX+0x20] __attribute__ ((aligned (32)));

qHD_Structure  Host20_qHD_List_Control0_P1 __attribute__ ((aligned (32)));
qHD_Structure  Host20_qHD_List_Control1_P1 __attribute__ ((aligned (32)))  ;
qHD_Structure  Host20_qHD_List_Bulk0_P1 __attribute__ ((aligned (32)));
qHD_Structure  Host20_qHD_List_Bulk1_P1 __attribute__ ((aligned (32))) ;
...
UINT8 *pUsbCtrlBufP1;
UINT8 UsbCtrlBufP1[0x100] __attribute__ ((aligned (8)));
UINT8  pUsbDataP1[128]  __attribute__ ((aligned (8))) ;
...
void USB_Bulk_Init2(struct usb_device *dev)
{
	...
	{//<5.1>.stop Asynchronous Schedule
		...
		pHost20_qHD_List_Control1_P1->bNextQHDPointer=(((UINT32)pHost20_qHD_List_Bulk0_P1)>>5);
		pHost20_qHD_List_Bulk0_P1->bNextQHDPointer=(((UINT32)pHost20_qHD_List_Bulk1_P1)>>5);
		pHost20_qHD_List_Bulk1_P1->bNextQHDPointer=(((UINT32)pHost20_qHD_List_Control0_P1)>>5);
	...
	wMaxPacketSize=ep->wMaxPacketSize;;
	pHost20_qHD_List_Bulk0_P1->bMaxPacketSize=wMaxPacketSize;
	pHost20_qHD_List_Bulk1_P1->bMaxPacketSize=wMaxPacketSize; //Suppose In/Out have the same Max packet Size
...
UINT8 flib_OTGH_Init2(UINT8 wForDevice_B)
{
	...
	mwHost20_CurrentAsynchronousAddr_Set((UINT32)Host20_STRUCTURE_qHD_BASE_ADDRESS_P1);
...
void flib_Host20_StopRun_Setting2(UINT8 bOption)
{
	if (bOption==HOST20_Enable) {
		...
		printf("\n??? Input Error 'flib_Host20_StopRun_Setting'...");
		//while(1);
	}
}
...
void flib_Host20_Asynchronous_Setting2(UINT8 bOption)
{
	if (bOption==HOST20_Enable) {
		...
		printf("\n??? Input Error 'flib_Host20_Asynchronous_Setting'...\n");
		// while(1);
	}
}
...
UINT8 flib_Host20_PortBusReset2(void)
{
	...
	while (1)
	{
		...
		if (wTmp>20000)
		{
			printf("\n??? Error waiting for Bus Reset Fail...==> Reset HW Control\n");
...
UINT8 flib_Host20_Issue_Control2(UINT8 bEdNum,UINT8* pbCmd,UINT16 hwDataSize,UINT8* pbData)
{
	...
	spTempqTD->ArrayBufferPointer_Word[0]=(UINT32)pUsbCtrlBufP1;
	...
	//earnest LGE
	bReturnValue=flib_Host20_Send_qTD2(spTempqTD ,qh_ptr,Host20_CtrTimeout);
...
UINT8  flib_Host20_Issue_Bulk2 (UINT8 bArrayListNum,UINT16 hwSize,UINT32 pwBuffer,UINT8 bDirection)
{
	...
	if (hwSize > (0x1000+0x1000-(pwBufferArray&0xfff))
			...
			spTempqTD->ArrayBufferPointer_Word[0] = workbuf;
			...
			{
				addr = workbuf;
				spTempqTD->ArrayBufferPointer_Word[i] = addr;
				//      		lastnums++;
				workbuf += 0x1000;
...
void flib_Host20_InitStructure2(void)
{
	...
	pUsbCtrlBufP1= KSEG02KSEG1(UsbCtrlBufP1);

	//ASSERT(pUsbCtrlBuf != NULL)

	qTD_Base_Buf_P1= KSEG02KSEG1(QtdBufP1);
...
#if 1
	pHost20_qHD_List_Control0_P1->bNextQHDPointer=(((UINT32)pHost20_qHD_List_Control1_P1)>>5);
	pHost20_qHD_List_Control1_P1->bNextQHDPointer=(((UINT32)pHost20_qHD_List_Control0_P1)>>5);
...
void flib_Host20_Allocate_QHD2(qHD_Structure  *psQHTemp,UINT8 bNextType,UINT8 bAddress,UINT8 bHead,UINT8 bEndPt, UINT32 wMaxPacketSize)
{
	...
	psQHTemp->bOverlay_NextqTD=(((UINT32)spTempqTD)>>5);
...
UINT8 flib_Host20_CheckingForResult_QHD2(qHD_Structure *spTempqHD)
{
	...
	{
		//printf("\n??? qHD Status => Halted (<1>.Stall/<2>.Babble/<3>.Error Counter=0)...(Device Not Supported...)\n");
	...
	{
		// printf("\n??? qHD Status => HOST20_qTD_STATUS_Babble...\n");
	...
	{
		//printf("\n??? qHD Status => HOST20_qTD_STATUS_TransactionError...\n");
		gUsbStatusP1|=USB_ST_CRC_ERR;
	}
	if (bQHStatus&HOST20_qTD_STATUS_MissMicroFrame)
	{
		//  printf("\n??? qHD Status => HOST20_qTD_STATUS_MissMicroFrame...\n");
...
UINT8 flib_Host20_Send_qTD2(qTD_Structure  *spHeadqTD ,qHD_Structure  *spTempqHD,int wTimeOutSec)
{
	...
	spOldDumyqTD=(qTD_Structure*)(((UINT32)(spTempqHD->bOverlay_NextqTD))<<5);
	...
	while(spLastqTD->bTerminate==0) {
		spLastqTD=(qTD_Structure*)(((UINT32)(spLastqTD->bNextQTDPointer))<<5);
	};

	spLastqTD->bNextQTDPointer=((UINT32)spNewDumyqTD)>>5;
	...
	spLastqTD->bAlternateQTDPointer=((UINT32)spNewDumyqTD)>>5;
...
//	mdelay(50);
	...
	while (1)
	{
		...
		if (mwHost20_PORTSC_ConnectStatus_Rd()==0) return HOST20_FATAL;
	...
	do {
		spReleaseqTDNext=(qTD_Structure*)(((UINT32)(spReleaseqTD->bNextQTDPointer))<<5);
