...
UINT8 QtdBuf[Host20_qTD_SIZE*Host20_qTD_MAX+0x20]	__attribute__ ((aligned (32)));
...
UINT8 *pUsbCtrlBuf;
UINT8 UsbCtrlBuf[0x100]			__attribute__ ((aligned (8)));
UINT8 pUsbData[128]				__attribute__ ((aligned (8)));
...
void USB_Bulk_Init(struct usb_device *dev)
{
	...
	{//<5.1>.stop Asynchronous Schedule
		...
		pHost20_qHD_List_Control1->bNextQHDPointer=(((UINT32)pHost20_qHD_List_Bulk0)>>5);
		pHost20_qHD_List_Bulk0->bNextQHDPointer=(((UINT32)pHost20_qHD_List_Bulk1)>>5);
		pHost20_qHD_List_Bulk1->bNextQHDPointer=(((UINT32)pHost20_qHD_List_Control0)>>5);
...
void flib_Host20_Periodic_Setting(UINT8 bOption)
{
	...
	{
		printf("\n??? Input Error 'flib_Host20_Periodic_Setting'...\n");
	}
}
...
void  flib_Host20_Issue_Interrupt(UINT8 bArrayListNum,UINT8 *buf,UINT16 hwSize)
{
	...
	spTempqTD->ArrayBufferPointer_Word[0]=(UINT32)(buf);        //cache issue, if cache enable
...
UINT8 flib_OTGH_Init(UINT8 wForDevice_B)
{
	...
	mwHost20_CurrentAsynchronousAddr_Set((UINT32)Host20_STRUCTURE_qHD_BASE_ADDRESS);
...
UINT8 flib_Host20_Issue_Control (UINT8 bEdNum,UINT8* pbCmd,UINT16 hwDataSize,UINT8* pbData)
{
	...
	spTempqTD->ArrayBufferPointer_Word[0]=(UINT32)pUsbCtrlBuf;
	...
	//earnest LGE
	bReturnValue=flib_Host20_Send_qTD(spTempqTD ,qh_ptr,Host20_CtrTimeout);
...
void flib_Host20_InitStructure(void)
{
	...
	pUsbCtrlBuf= KSEG02KSEG1(UsbCtrlBuf);

	//ASSERT(pUsbCtrlBuf != NULL)

	qTD_Base_Buf= KSEG02KSEG1(QtdBuf);
...
#if 1
	pHost20_qHD_List_Control0->bNextQHDPointer=(((UINT32)pHost20_qHD_List_Control1)>>5);
	pHost20_qHD_List_Control1->bNextQHDPointer=(((UINT32)pHost20_qHD_List_Control0)>>5);
...
UINT8 flib_Host20_CheckingForResult_QHD(qHD_Structure *spTempqHD)
{
	...
	if (bQHStatus&HOST20_qTD_STATUS_BufferError)
	{
		...
		//printf("\n??? qHD Status => HOST20_qTD_STATUS_BufferError...\n");
	}
