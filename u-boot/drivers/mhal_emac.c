...
//#include <linux/pci.h>
#include <asm/types.h>  //2008/12/03 Nick
#include "mhal_emac.h"
...
void MHal_EMAC_read_phy(unsigned char phy_addr, unsigned char address,u32 *value)
{
   ...
   uRegVal = (PHY_LOW_HIGH<<PHY_LOW_HIGH_SHIFT) | (PHY_READ_OP<<PHY_RW_SHIFT) |
                    (phy_addr<<PHY_ADDR_SHIFT) | (address<<PHY_REG_SHIFT) |
...
u32 MHal_EMAC_NegotiationPHY(void)
{
	...
	//printf("MHal_EMAC_Read_CTL is %4x \n",word_ETH_CTL);
    ...
    word_PHY = 0x67FE0000;// Read for 32th reg
