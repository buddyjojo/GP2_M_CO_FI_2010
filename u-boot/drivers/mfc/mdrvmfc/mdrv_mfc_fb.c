...
void MDrv_MFC_SetMiuSSC(U16 u16KHz, U16  u16Percent, BOOL bEnable)
{
    ...
    if(bEnable)
    {
       ...
       // printf("\n\r\n\r u8DDRPLL_INPUT_DIV_2=[0x%x]  ", u8DDRPLL_INPUT_DIV_2);
        ...
        if(u8DDRPLL_INPUT_DIV_1&&u8DDRPLL_INPUT_DIV_2)
        {
            ...
            if(u16DDFSET)
            {
                ...
                if(u8DDFM&&u16KHz)
                {
                    ...
                    if(u16Span)
                    {
                        ...
                        MDrv_MFC_Write2Bytes(0x132A, (u16Span&0x03FF));
    ...
    else  // this to skip setting when divide by zero.
    {
    // disable
        MDrv_MFC_WriteBit(0x1329, _DISABLE, _BIT7);
    MDrv_MFC_Write2Bytes(0x132A, 0);
    MDrv_MFC_WriteByte(0x1328, 0);
}
}
...
#define MIU_SSC_SPAN_DEFAULT        300
...
void MDrv_MFC_InitializeMiu(void)
{
	...
	else //DDR3
	{
        ...
        if(S7M)
        {
            ...
            MDrv_MFC_WriteByte(0x1226, 0x66);
            MDrv_MFC_WriteByte(0x1227, 0x33);
        }
        else
        {
            MDrv_MFC_WriteByte(0x1212, 0x06);
            MDrv_MFC_WriteByte(0x1226, 0x44);
            MDrv_MFC_WriteByte(0x1227, 0x00);
        }
        ...
        if( IS_CHIP_3xxx_U02_AND_AFTER(gmfcSysInfo.u8ChipRevision) )
		{
            ...
            #if NANYA_DDR_NEW_SETTING
            if(gmfcSysInfo.u8ChipRevision<=MST_3xxx_U02)
            #else
            if(1)
            #endif
                {
