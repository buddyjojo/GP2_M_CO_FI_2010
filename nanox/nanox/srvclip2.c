...
GsSetClipWindow(GR_WINDOW *wp, MWCLIPREGION *userregion, int flags)
{
	...
	while (pwp != NULL) {
		...
		for (; sibwp != wp; sibwp = sibwp->siblings) {
			...
			/* Added by T.J.Park to implement transparent windows */
			if(sibwp->props & GR_WM_PROPS_TRANSPARENT)
			{
				/* This is a transparent sibling. Do not clip this one. Instead, this sibling must be redrawn. */
				/* Application programmer must redraw this sibling. */
				continue;
			}
	 ...
	 */
	if(wp != rootwp[layer] && !(flags & GR_MODE_EXCLUDECHILDREN)) {
