...
#if FONT_INSTALLER
#include "../engine/font_data_table.h"
#endif

#if HAVE_FONTFUSION_SUPPORT
#include "../engine/font_fontfusion.h"
#endif

#if HAVE_UNITYPE_SUPPORT
#include "../engine/font_unitype.h"
#endif

extern	PMWFONT	gr_pfont;

static int	nextid = GR_ROOT_WINDOW_ID + 10;

static void CheckNextEvent(GR_EVENT *ep, GR_BOOL doCheckEvent);

#if	(SYS_DVB)
/*
 * Added by Henry for Chinese UNICODE display @20070714
 */
int
GrAll2GBK(const void *istr, void *ostr)
{
	int icc = 0;
	icc =  GdAll2GBK(istr, ostr);
	return icc;
}
#endif	/* (SYS_DVB) */
...
GrGetScreenInfo(GR_SCREEN_INFO *sip)
{
	...
	GdGetScreenInfo(rootwp[0]->psd, sip);
...
/*
 * T.J.Park : Return information about the "specified" screen for clients to use.
 */
void
GrGetScreenInfoEx(int layer, GR_SCREEN_INFO *sip)
{
	...
	if(layer < scrNumberOfLayers)
	{
		GdGetScreenInfo(rootwp[layer]->psd, sip);
...
GrSetScreenResolution(int width, int height)
{
	...
	GdSetScreenRes(rootwp[0]->psd, width, height);
	rootwp[0]->width = width;
	rootwp[0]->height = height;
...
GrSetScreenResolutionEx(int layer, int width, int height)
{
	...
	if(layer < scrNumberOfLayers)
	{
		GdSetScreenRes(rootwp[layer]->psd, width, height);
		rootwp[layer]->width = width;
		rootwp[layer]->height = height;
	}
	SERVER_UNLOCK();
}
...
GR_WINDOW_ID
GrFindVisibleWindow(GR_COORD x, GR_COORD y)
{
	GR_WINDOW * pWinInfo = GsFindVisibleWindow(x, y);
	return pWinInfo->id;
}
...
OffsetWindow(GR_WINDOW *wp, GR_COORD offx, GR_COORD offy)
{
	...
	wp->x = SAFE_PLUS(wp->x, offx);
	wp->y = SAFE_PLUS(wp->y, offy);
...
GrMoveWindow(GR_WINDOW_ID wid, GR_COORD x, GR_COORD y)
{
	...
	if (wp == rootwp[wp->layer]) {
		GsError(GR_ERROR_ILLEGAL_ON_ROOT_WINDOW, wid);
		SERVER_UNLOCK();
		return;
	}

	x = SAFE_PLUS(x, wp->parent->x);
	y = SAFE_PLUS(y, wp->parent->y);
	offx = x - wp->x;
	offy = y - wp->y;
...
#endif
	/* T.J.Park Note : Because we are using off-screen, the following move-algorithm is faster and
	   also not flickering. */
#if 1
	...
	if (wp->mapped && wp == wp->parent->children) {
		...
		X = MWMIN(oldx, wp->x);
		Y = MWMIN(oldy, wp->y);
		W = SAFE_PLUS(MWMAX(oldx, wp->x), wp->width) - X;
		H = SAFE_PLUS(MWMAX(oldy, wp->y), wp->height) - Y;
		GsExposeArea(rootwp[wp->layer], X, Y, W, H, NULL);
		DeliverUpdateMoveEventAndChildren(wp);
		SERVER_UNLOCK();
		return;
	}
...
GrNewGC(void)
{
	...
	gcp = (GR_GC *) NANOX_MALLOC(sizeof(GR_GC));
	if (gcp == NULL) {
	...
	gcp->gradation.start_pos = -1;
	gcp->gradation.length = -1;
	gcp->gradation.direction = GRADATION_NONE;
...
GrNewRegion(void)
{
	...
	regionp = (GR_REGION *) NANOX_MALLOC(sizeof(GR_REGION));
	if (regionp == NULL) {
		GsError(GR_ERROR_MALLOC_FAILED, 0);
		SERVER_UNLOCK();
		return 0;
...
{
	SERVER_LOCK();
	if(!FDT_FindFontData(name, NULL, NULL, NULL))
	{
		FDT_InstallFontData(name, data, length, 0);
	}
	SERVER_UNLOCK();
}
...
{
	SERVER_LOCK();
	if(FDT_UninstallFontData(name))
	{
#if HAVE_FONTFUSION_SUPPORT
		/* Remove all the font data created with the given name. */
		GR_FONT		*fontp;
		GR_FONT		*prevfontp;
		GR_CHAR		fontname[MWLF_FACESIZE];

		while(1)
		{
			/* Find the font with the name */
			fontp = listfontp;
			while(fontp)
			{
				if(fontfusion_getfontname(fontp->pfont, fontname)
				&& strcmp(name, fontname) == 0)
					break; /* bingo */
			...
			if(fontp == NULL)
			{ /* can not find a font with the name any more */
				break;
			}

			/* Now remove it. This routine is copied from GrDestroyFont. */
			if (listfontp == fontp)
			...
			/* T.J.Park Added */
			if(gr_pfont == fontp->pfont)
			{ /* The font is being used now. */
				GdSetFont(NULL);
			}
...
GrCreateFont(GR_CHAR *name, GR_COORD height, GR_LOGFONT *plogfont)
{
	...
	fontp = (GR_FONT *) NANOX_MALLOC(sizeof(GR_FONT));
	if (fontp == NULL) {
		GsError(GR_ERROR_MALLOC_FAILED, 0);
		SERVER_UNLOCK();
		return 0;
...
void
GrSetFontPitch(GR_FONT_ID fontid, int pitchval)
{
#if HAVE_FONTFUSION_SUPPORT
	GR_FONT		*fontp;
...
{
#if HAVE_UNITYPE_SUPPORT
	return UNITYPE_GetIsUTFFont();
#else
	return 1;	//다른 engine에서도 사용이 필요하다면 추가 구현해서 사용(091023)
#endif
}
...
GrSelectEvents(GR_WINDOW_ID wid, GR_EVENT_MASK eventmask)
{
	 ...
	 */
	evp = (GR_EVENT_CLIENT *) NANOX_MALLOC(sizeof(GR_EVENT_CLIENT));
	if (evp == NULL) {
	 ...
	 */
	if (wp->parent == NULL && (eventmask & GR_EVENT_MASK_CHLD_UPDATE)) {
		for (wp = listwp[wp->layer]; wp; wp = wp->next) {
			if (wp->realized)
	...
	GR_COLOR bordercolor)
{
	...
	wp->id = nextid++;
	if(nextid == 0)
		nextid = GR_ROOT_WINDOW_ID + 10;

	wp->psd = pwp->psd;
	wp->parent = pwp;
	wp->children = NULL;
	wp->siblings = pwp->children;
	wp->next = listwp[pwp->layer];
	wp->x = pwp->x + x;
	wp->y = pwp->y + y;
	wp->width = width;
	wp->height = height;
...
GR_WINDOW_ID
GrNewPixmapEx(GR_SIZE width, GR_SIZE height, void *pixels, int layer)
{
	GR_PIXMAP	*pp;
	PSD psd;
	int size, linelen, bpp, planes, stride;
	GR_WINDOW_ID id;
	 ...
	 */
	planes = rootwp[layer]->psd->planes;
	bpp = rootwp[layer]->psd->bpp;
	psd = rootwp[layer]->psd->AllocateMemGC(rootwp[layer]->psd);
	if (!psd) {
		SERVER_UNLOCK();
		return 0;
	}

	pp = (GR_PIXMAP *) NANOX_MALLOC(sizeof(GR_PIXMAP));
	if (pp == NULL) {
		GsError(GR_ERROR_MALLOC_FAILED, 0);
		psd->FreeMemGC(psd);
		SERVER_UNLOCK();
	...
	if (!pixels) {
#if 0
		pixels = NANOX_CALLOC(size, 1);
#else
		stride = 0;
		pixels = NANOX_ScrAllocateFrame(psd->bpp, width, height, size, &stride);
		if (stride) {
			size = height*stride;
			switch (psd->bpp) {
				case 16 :
...
{
	return GrNewPixmapEx(width, height, pixels, 0);
}
	...
	GR_BITMAP *bgbitmap)
{
	...
	cp = (GR_CURSOR *)NANOX_MALLOC(sizeof(GR_CURSOR));
	if (cp == NULL) {
		GsError(GR_ERROR_MALLOC_FAILED, 0);
		SERVER_UNLOCK();
		return 0;
...
/*
 * Set the global alpha value in a graphics context.
 * Global alpha value overrides the alpha channel of each pixel
 * when the drawing mode is source-over mode.
 */
void
GrSetGCAlpha(GR_GC_ID gc, int alpha)
{
	GR_GC		*gcp;		/* graphics context */
...
/*
 * Set the gradiation start option. If the user wants to draw text in gradiation mode, use this value.
 */
void
GrSetGCGradationOption(GR_GC_ID gc, int startpos, int length, int direction)
{
	GR_GC		*gcp;		/* graphics context */
	...
	gcp = GsFindGC(gc);
	if (!gcp || (gcp->gradation.start_pos == startpos && gcp->gradation.length == length && gcp->gradation.direction == direction)) {
		SERVER_UNLOCK();
		return;
	}

	gcp->gradation.start_pos = startpos;
	gcp->gradation.length = length;
	gcp->gradation.direction = direction;
	gcp->changed = GR_TRUE;
...
/*
 * Set the xorcolor in a graphics context from a passed RGB color value. added by foryou
 */
void
GrSetGCXor(GR_GC_ID gc, GR_COLOR xorcolor)
{
	GR_GC		*gcp;		/* graphics context */
	...
	gcp = GsFindGC(gc);
	if (!gcp || gcp->xorcolor == xorcolor) {
		SERVER_UNLOCK();
		return;
	}
	gcp->xorcolor = xorcolor;
	gcp->changed = GR_TRUE;
	SERVER_UNLOCK();
}
...
GrSetGCStipple(GR_GC_ID gc, GR_BITMAP * bitmap, GR_SIZE width, GR_SIZE height)
{
	...
	gcp->stipple.bitmap =
		NANOX_MALLOC(GR_BITMAP_SIZE(width, height) * sizeof(GR_BITMAP));
...
/*
 * Get the text font from graphics context.
 * Added by Henry@20070919
 */
GR_FONT_ID
GrGetGCFont(GR_GC_ID gc)
{
	GR_GC		*gcp;
	GR_FONT_ID 	font = 0;
	...
	GR_COORD y2)
{
	...
	switch (GsPrepareDrawing(id, gc, &dp)) {
		...
		case GR_DRAW_TYPE_PIXMAP:
			/* FIXME : T.J.Park : To avoid overflow, I put the following code before adding dp->x and dp->y.
								  This is incorrect, but acceptable. */
			if(x1 > MAX_MWCOORD)
				x1 = MAX_MWCOORD;
			else if(x1 < MIN_MWCOORD)
				x1 = MIN_MWCOORD;

			if(y1 > MAX_MWCOORD)
				y1 = MAX_MWCOORD;
			else if(y1 < MIN_MWCOORD)
				y1 = MIN_MWCOORD;
	...
	GR_SIZE ry)
{
	...
	switch (GsPrepareDrawing(id, gc, &dp)) {
		...
		case GR_DRAW_TYPE_PIXMAP:
			/* FIXME : T.J.Park : To avoid overflow, I put the following code before adding dp->x and dp->y.
								  This is incorrect, but I don't have better idea. */
			if(GdClipArc(&x, &y, &rx, &ry
				, 0, 0, dp->width - 1, dp->height - 1))
			{
...
GrLoadImageFromFile(char *path, int flags)
{
	...
	imagep = (GR_IMAGE *) NANOX_MALLOC(sizeof(GR_IMAGE));
	if (!imagep) {
		GsError(GR_ERROR_MALLOC_FAILED, 0);
		GdFreeImage(id);
		SERVER_UNLOCK();
...
void
GrDrawImageFromRawDataBuffer(GR_DRAW_ID id, GR_GC_ID gc, GR_COORD x, GR_COORD y,
	GR_SIZE width, GR_SIZE height, void *buffer, GR_SIZE buffer_width, GR_SIZE buffer_height)
{
	...
	case GR_DRAW_TYPE_WINDOW:
	case GR_DRAW_TYPE_PIXMAP:
		GdDrawImageFromRawDataBuffer(dp->psd, dp->x + x, dp->y + y,
			width, height, buffer, buffer_width, buffer_height);
		break;
	}
...
void
GrLoadImageEx(void *buffer, int size, MW_LOAD_IMAGE_EX_RESULT *pResult)
{
	if(buffer == NULL || size == 0 || pResult == NULL)
		return;

	SERVER_LOCK();

	GdLoadImageEx(scrdev, buffer, size, pResult);
...
/** T.J.Park : This function is added to support scissoring and stretching of an image.
              This is a JavaVM requirement. The following two function calls make exactly same result.
	<PRE>
						...
						, GR_COORD imageWidth, GR_COORD imageHeight)
{
	...
	switch (GsPrepareDrawing(id, gc, &dp)) {
		case GR_DRAW_TYPE_WINDOW:
		case GR_DRAW_TYPE_PIXMAP:
			GdDrawImageToFitEx(dp->psd, dp->x + x, dp->y + y,
				width, height, imageid, imageX, imageY, imageWidth, imageHeight);
			break;
	...
	for (imagep = listimagep; imagep; imagep = imagep->next) {
		if (imagep->id == id) {
			...
			if (listimagep == imagep)
				...
				while (previmagep && previmagep->next != imagep)
...
GrPoly(GR_DRAW_ID id, GR_GC_ID gc, GR_COUNT count, GR_POINT *pointtable)
{
	GR_DRAWABLE	*dp;
	GR_POINT	*pp;
	GR_POINT	*srcPointTable;
	GR_POINT	*dstPointTable;
	GR_COUNT	i;
	PSD 		psd;
	 ...
	 */
	pp = NANOX_MALLOC(sizeof(GR_POINT) * count);
	if(!pp)
	{
		SERVER_UNLOCK();
		return;
	...
	i = count;
	srcPointTable = pointtable;
	dstPointTable = pp;
	while(i--)
	{
		/* FIXME : T.J.Park : To avoid overflow, I put the following code before adding dp->x and dp->y.
		                      This is incorrect, but acceptable. */
		if(srcPointTable->x > MAX_MWCOORD)
			dstPointTable->x = MAX_MWCOORD;
		else if(srcPointTable->x < MIN_MWCOORD)
			dstPointTable->x = MIN_MWCOORD;
		else
			dstPointTable->x = srcPointTable->x;

		if(srcPointTable->y > MAX_MWCOORD)
			dstPointTable->y = MAX_MWCOORD;
		else if(srcPointTable->y < MIN_MWCOORD)
			dstPointTable->y = MIN_MWCOORD;
		else
			dstPointTable->y = srcPointTable->y;

		dstPointTable->x += dp->x;
		dstPointTable->y += dp->y;
...
/* Return the system palette entries of the specified screen */
void
GrGetSystemPaletteEx(int layer, GR_PALETTE *pal)
{
	...
	if(rootwp[layer]->psd->pixtype == MWPF_PALETTE) {
		pal->count = (int)rootwp[layer]->psd->ncolors;
		GdGetPalette(rootwp[layer]->psd, 0, pal->count, pal->palette);
	}
...
{
	GrGetSystemPaletteEx(0, pal);
}

/* Set the system palette entries of the specified screen from first for count*/
void
GrSetSystemPaletteEx(int layer, GR_COUNT first, GR_PALETTE *pal)
{
	SERVER_LOCK();

	GdSetPalette(rootwp[layer]->psd, first, pal->count, pal->palette);
	if (first == 0)
...
{
	GrSetSystemPaletteEx(0, first, pal);
}

/* Convert passed color value to pixel value, depending on system mode*/
void
GrFindColorEx(int layer, GR_COLOR c, GR_PIXELVAL *retpixel)
{
	SERVER_LOCK();
	*retpixel = GdFindColor(scrdev + layer, c);
	SERVER_UNLOCK();
}
...
#ifdef VXWORKS /* Vxworks does not support strdup(). */

char *strdup(const char *sz)
{
	char *p;

	p = malloc(strlen(sz) + 1);
	if(p == NULL)
		return NULL;
...
GrSetSelectionOwner(GR_WINDOW_ID wid, GR_CHAR *typelist)
{
	...
	if(selection_owner.typelist) NANOX_FREE(selection_owner.typelist);
...
/*
 * Copy and stretch a rectangle. Does not support flip.
 */
void
GrStretchArea(GR_DRAW_ID dstid, GR_GC_ID gc,
	      GR_COORD dx, GR_COORD dy,
	      GR_COORD dw, GR_COORD dh,
	      GR_DRAW_ID srcid,
	      GR_COORD sx, GR_COORD sy,
	      GR_COORD sw, GR_COORD sh,
	      unsigned long op)
{
	GR_DRAWABLE *dp;
	GR_WINDOW *swp;
...
/*
 * Copy and stretch/flip a rectangle.
 */
void
GrStretchAreaEx(GR_DRAW_ID dstid, GR_GC_ID gc,
...
nochildren:
	...
	*parentid = wp->parent? wp->parent->id: 0;
	...
	retarray = (GR_WINDOW_ID *)NANOX_MALLOC(n * sizeof(GR_WINDOW_ID));
	if (!retarray) {
		GsError(GR_ERROR_MALLOC_FAILED, 0);
		goto nochildren;
	}
...
GrCreateTimer (GR_WINDOW_ID wid, GR_TIMEOUT period)
{
    ...
    timer = (GR_TIMER*) NANOX_MALLOC (sizeof (GR_TIMER));
    if (timer == NULL)
    {
        GsError (GR_ERROR_MALLOC_FAILED, 0);
		SERVER_UNLOCK();
...
GrNewBitmapRegion(GR_BITMAP *bitmap, GR_SIZE width, GR_SIZE height)
{
	...
	if (!(regionp = NANOX_MALLOC(sizeof(GR_REGION)))) {
		GsError(GR_ERROR_MALLOC_FAILED, 0);
		SERVER_UNLOCK();
		return 0;
	}
...
GrGrabKey(GR_WINDOW_ID id, GR_KEY key, int type)
{
	...
	keygrab = (GR_GRABBED_KEY *)NANOX_MALLOC(sizeof(GR_GRABBED_KEY));
	if (keygrab == NULL) {
		GsError(GR_ERROR_MALLOC_FAILED, 0);
		SERVER_UNLOCK();
		return GR_FALSE;
...
void
GrCopyOffscreenEx(int layer, GR_REGION_ID rid)
{
	GR_REGION *regionp;

	SERVER_LOCK();
	regionp = GsFindRegion(rid);
	if(regionp == NULL)
	{
		GdCopyOffscreenEx(layer, NULL);
	}
	else
	{
		GdCopyOffscreenEx(layer, regionp->rgn);
	}
	SERVER_UNLOCK();
}

void
GrCopyOffscreen(GR_REGION_ID rid)
{
	GrCopyOffscreenEx(0, rid);
}


void
GrCacheFont(GR_GC_ID gc, void *str, int count)
{
	GR_GC		*gcp;
	GR_FONT 	*fontp;
	PMWFONT 	pf;
	...
	pf = fontp ? fontp->pfont: stdfont;
#if HAVE_FONTFUSION_SUPPORT
	fontfusion_cachefont(pf, str, count);
#endif
#if HAVE_UNITYPE_SUPPORT
	UNITYPE_CacheFont(pf, str, count);
#endif
