...
#define SERVER_LOCK()         if(!gr_server_mutex) {gr_server_mutex = OSA_CreateMtxSem("nanoxMtx", SMF_RECURSIVE);} \
								OSA_WaitSem(gr_server_mutex, OSA_WAIT_FOREVER)
#define SERVER_UNLOCK()       if(!gr_server_mutex) {gr_server_mutex = OSA_CreateMtxSem("nanoxMtx", SMF_RECURSIVE);} \
								OSA_PostSem(gr_server_mutex)
#else
...
#endif
#else
/* T.J.Park Note : In most systems(except Linux), mutex or semaphore has a plain type(normaly UINT32), not structure.
   So, we don't need to put the pointer of the variable. */
#define SERVER_LOCK_INIT()    LOCK_INIT(gr_server_mutex)
#define SERVER_LOCK()         LOCK(gr_server_mutex)
#define SERVER_UNLOCK()       UNLOCK(gr_server_mutex)
#endif

/* T.J.Park Note : Why does not SERVER_LOCK_FREE() here??? Maybe the author thought it is not necessary.
   OK. At anyway, we don't call GrOpen() and GrClose() repeatedly. */
...
struct gr_gc {
	...
	int				alpha;			/* global alpha, added by T.J.Park */
	MWGRADATION	gradation;	/* gradiation option, added by T.J.Park */
	...
	GR_COLOR		xorcolor;		/* xorcolor added by foryou */
...
struct gr_window {
	...
	int			layer;		/* T.J.Park Note : Add a variable to specify the OSD layer where this window exists
								Ex: 0 for OSD_LAYER_1, 1 for OSD_LAYER_2... */
...
void		GsDeliverGeneralEvent(GR_WINDOW *wp, GR_EVENT_TYPE type,
			GR_WINDOW *other);
void		GsDeliverPortraitChangedEvent(int layer);
...
extern	GR_WINDOW	*listwp[MAX_OSD_LAYER];		/* list of all windows */
extern	GR_WINDOW	*rootwp[MAX_OSD_LAYER];		/* root window pointer */
