 ...
 */
void
GsRedrawScreen(int layer)
{
	/* Redraw all windows*/
	GsExposeArea(rootwp[layer], 0, 0, rootwp[layer]->width, rootwp[layer]->height, NULL);
}
...
GsWpDestroyWindow(GR_WINDOW *wp)
{
	...
	if(selection_owner.wid == wp->id) {
		...
		if(selection_owner.typelist)
			NANOX_FREE(selection_owner.typelist);
...
GsFindWindow(GR_WINDOW_ID id)
{
	...
	for(layer = 0;layer < scrNumberOfLayers;layer++)
	{
		for (wp = listwp[layer]; wp; wp = wp->next) {
			if (wp->id == id) {
				cachewindowid = id;
				cachewp = wp;
				return wp;
...
GsPrepareDrawing(GR_DRAW_ID id, GR_GC_ID gcid, GR_DRAWABLE **retdp)
{
	...
	*retdp = NULL;
	...
	if (gcp->changed) {
		...
		GdSetGlobalAlpha(gcp->alpha);
		if(wp != NULL) {
			if(gcp->gradation.direction == GRADATION_UP || gcp->gradation.direction == GRADATION_DOWN)	gcp->gradation.start_pos += wp->y;
			if(gcp->gradation.direction == GRADATION_LEFT || gcp->gradation.direction == GRADATION_RIGHT)	gcp->gradation.start_pos += wp->x;
		}
		GdSetGradationOption(gcp->gradation);

		/* added by foryou */
		GdSetXorColor(psd, gcp->xorcolor);
...
GsCheckFocusWindow(void)
{
	...
	for (wp = mousewp; ;wp = wp->parent) {
		...
		if ((wp == rootwp[0]) || (wp->nopropmask & GR_EVENT_MASK_KEY_DOWN)) {
			GsWpSetFocus(rootwp[0]);
			return;
		}
	}
}
 ...
 */
void
GsSetPortraitModeEx(int layer, int mode)
{
	PSD psd = rootwp[layer]->psd;

	GdSetPortraitMode(psd, mode);
	GdRestrictMouse(0, 0, psd->xvirtres - 1, psd->yvirtres - 1);

	/* reset clip and root window size*/
	clipwp = NULL;
	rootwp[layer]->width = psd->xvirtres;
	rootwp[layer]->height = psd->yvirtres;

	/* deliver portrait changed event to all windows selecting it*/
	GsDeliverPortraitChangedEvent(layer);
 ...
 */
void
GsSetPortraitModeFromXYEx(int layer, GR_COORD rootx, GR_COORD rooty)
{
	int newmode;
	PSD psd = rootwp[layer]->psd;
	...
	if (rootx == 0) {
		...
		GsSetPortraitModeEx(layer, newmode);
		GrMoveCursor(5, rooty);
		GdMoveMouse(5, rooty);
	} else if (rootx == psd->xvirtres-1) {
		/* rotate right*/
		switch (psd->portrait) {
		case MWPORTRAIT_NONE:
		default:
		...
		GsSetPortraitModeEx(0, newmode);
		GrMoveCursor(psd->xvirtres-5, rooty);
		GdMoveMouse(psd->xvirtres-5, rooty);
	}
}
...
{
	GsSetPortraitModeFromXYEx(0, rootx, rooty);
}
