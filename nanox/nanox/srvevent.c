...
GR_EVENT *GsAllocEvent(GR_CLIENT *client)
{
	...
	if (elp)
		...
		elp = (GR_EVENT_LIST *) NANOX_MALLOC(sizeof(GR_EVENT_LIST));
		if (elp == NULL) {
			...
			int modifiers)
{
	...
	for (;;) {
		...
		if ((wp == rootwp[wp->layer]) || (wp->nopropmask & eventmask))
			return;
...
void GsDeliverMotionEvent(GR_EVENT_TYPE type, int buttons, MWKEYMOD modifiers)
{
	...
	for (;;) {
		...
		if ((wp == rootwp[wp->layer]) || grabbuttonwp ||
	...
	GR_KEY keyvalue, GR_KEYMOD modifiers, GR_SCANCODE scancode)
{
	...
	if (keygrab != NULL) {
		...
		kwp = focuswp;
		while (kwp != wp && kwp != rootwp[kwp->layer])
			kwp = kwp->parent;
		 ...
		 */
		tempwp = wp;
		while (tempwp != kwp && tempwp != rootwp[tempwp->layer])
 ...
 */
void
GsDeliverPortraitChangedEvent(int layer)
{
	GR_WINDOW		*wp;
	GR_EVENT_GENERAL	*gp;
	GR_EVENT_CLIENT		*ecp;
...
GsDeliverScreenSaverEvent(GR_BOOL activate)
{
	...
	/* FIXME : T.J.Park : rootwp[0] receives this event always. */
	for (ecp = rootwp[0]->eventclients; ecp; ecp = ecp->next) {
	...
	GR_SERIALNO serial, GR_LENGTH len, GR_LENGTH thislen, void *data)
{
	...
	for (ecp = wp->eventclients; ecp; ecp = ecp->next) {
		...
		gp->len = len;
		gp->datalen = thislen;
		if(!(gp->data = NANOX_MALLOC(thislen))) {
...
GsDeliverRawMouseEvent(int rx, int ry, int buttons, int modifiers)
{
	...
	for (i = 0; i < 2; i++) {
		...
		for (ecp = wp->eventclients; ecp; ecp = ecp->next) {
			...
			if ((wp == rootwp[wp->layer]) || (wp->nopropmask & GR_EVENTMASK(etype)))
				break;
