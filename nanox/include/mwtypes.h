...
#define MWTF_SIDEBUG	0x00000020L	/* SI debuging */
...
#define MWTF_SKIP_DINGBAT	0x00010000L	/* skip dingbat font */

/* Draw Text prop. flags*/
#define MWTF_GRADATION_UP	0x00100000L	/* font gradation to up */
#define MWTF_GRADATION_DOWN	0x00200000L	/* font gradation to down */
#define MWTF_GRADATION_LEFT	0x00300000L	/* font gradation to left */
#define MWTF_GRADATION_RIGHT	0x00400000L	/* font gradation to right */
#define MWTF_GRADATIONMASK	0x00700000L	/* font gradation direction mask */
#define MWTF_LIGHT_GRADATION	0x00800000L	/* font gradation weak */
...
#define MWTF_ITALIC		0x0008		/* italic */
#define MWTF_BOLD		0x0010		/* bold */
#endif
...
/* Gradation */
#define GRADATION_NONE		0
#define GRADATION_UP			1
#define GRADATION_DOWN		2
#define GRADATION_LEFT		3
#define GRADATION_RIGHT		4
...
#define MWPF_TRUECOLOR1555 9	/* pixel is packed 16 bits 1/5/5/5 truecolor with alpha : added 2007.03. */
...
#endif
#ifdef _16BPP_
#define MWPIXEL_FORMAT	MWPF_TRUECOLOR1555		// Modified 2007.03.
#endif
...
/* T.J.Park : User color's order is 0xAABBGGRR */
...
/* T.J.Park : Add a macro to make a system color. The order is 0xAARRGGBB. */
#define MW_SYS_ARGB(a,r,g,b)	((MWCOLORVAL)(((unsigned char)(b)|\
				(((unsigned)(unsigned char)(g))<<8))|\
				(((unsigned long)(unsigned char)(r))<<16)|\
				(((unsigned long)(unsigned char)(a))<<24)))

/* 2005-5-30. T.J.Park. A, R, G, B를 얻어내는 매크로 추가 */
#define MW_GET_R(color)		((unsigned char)(color))
#define MW_GET_G(color)		((unsigned char)((color) >> 8))
#define MW_GET_B(color)		((unsigned char)((color) >> 16))
#define MW_GET_A(color)		((unsigned char)((color) >> 24))

#define MW_SYS_GET_A(color)	((unsigned char)((color) >> 24))
#define MW_SYS_GET_R(color)	((unsigned char)((color) >> 16))
#define MW_SYS_GET_G(color)	((unsigned char)((color) >> 8))
#define MW_SYS_GET_B(color)	((unsigned char)(color))

#elif (MWPIXEL_FORMAT == MWPF_TRUECOLOR1555)	// added 2007.03.20
#define MWARGB(a,r,g,b)	((MWCOLORVAL)(((unsigned char)(r)|\
				(((unsigned)(unsigned char)(g))<<8))|\
				(((unsigned long)(unsigned char)(b))<<16)|\
				(((unsigned long)(unsigned char)(a))<<24)))
...
#define MW_SYS_ARGB(a,r,g,b)	((a) ? \
								((((unsigned short)b>>3)|((unsigned short)(g&0xf8)<<2)|((unsigned short)(r&0xf8)<<7)) ?\
								(((unsigned short)b>>3)|((unsigned short)(g&0xf8)<<2)|((unsigned short)(r&0xf8)<<7)) : 0x0001)\
								|((unsigned short)(a==0xff ? 0x0001:0x8001))\
								: 0)
//#define MW_SYS_ARGB(a,r,g,b)	((a) ? \
//								(((unsigned short)b>>3)|((unsigned short)(g&0xf8)<<2)|\
//								((unsigned short)(r&0xf8)<<7)|((unsigned short)(a==0xff ? 0x0001:0x0001)))\
//								: 0)
...
#define MW_SYS_GET_A(color)	((unsigned char)((color) ? ((color&0x8000) ? 0x80:0xff):0))
#define MW_SYS_GET_R(color)	((unsigned char)((color&0x7c00) >> 7))
#define MW_SYS_GET_G(color)	((unsigned char)((color&0x03e0) >> 2))
#define MW_SYS_GET_B(color)	((unsigned char)((color&0x001f) << 3))
...
#if (MWPIXEL_FORMAT == MWPF_TRUECOLOR565) || (MWPIXEL_FORMAT == MWPF_TRUECOLOR555) || (MWPIXEL_FORMAT == MWPF_TRUECOLOR1555)	// Added 2007.03.
...
#define MWIMAGE_REVERSE		0x40000000
...
#define MWLF_FACESIZE		256	/* max facename size*/
...
typedef struct {
	...
	long	lfWidth;		/* 현재 설정된 높이에 대한 비율로 정의, 0 or 100이면 default, 2006.05.26, Kim Jong Hyeon */
	long	lfEscapement;		/* rotation in tenths of degree*/
	long	lfOrientation;		/* not used*/
	long	lfWeight;		/* font weight*/
	long	lfPitchValue;		/* font pitch value */
	...
	MWUCHAR	lfKerning;		/* =1 for kerning */
...
#define MWLF_SetKerning(lf)			\
	do {							\
		(lf)->lfKerning = 1;		\
	} while (0)

#define MWLF_NoAntialias(lf)		\
	do {							\
		(lf)->lfQuality = 0;	\
	} while (0)

#define MWLF_Pitch(lf, value)			\
	do {								\
		(lf)->lfPitchValue = value;		\
	} while (0)

#define MWLF_Width(lf, value)			\
	do {								\
		(lf)->lfWidth = value;		\
	} while (0)

#define MWLF_Height(lf, value)			\
	do {								\
		(lf)->lfHeight= value;		\
	} while (0)

#define MWLF_Name(lf, value)			\
	do {								\
		strcpy((lf)->lfFaceName, value);	\
	} while (0)
...
/* T.J.Park Note : To increase the performance, LoadImage does not return upsidedown image,
   So this flag is obsolete.
   Actually, this flag was only used when loading TIFF image. */
#if 0
...
typedef struct {
	...
	/* Animated GIF only. Added by T.J.Park */
	int		log_screen_width;	/* Logical screen width of all images included in a GIF file */
	int		log_screen_height;	/* Logical screen height of all images included in a GIF file */
	int		xoffset;		/* x-offset of each GIF image, relative to the GIF screen */
	int		yoffset;		/* y-offset of each GIF image, relative to the GIF screen */
    int		delaytime;		/* the delay time until drawing the next image. This value is specified by millisecond, though GIF89 defines it as 1/100 seconds */
    int		inputflag;		/* Indicates whether or not user input is expected before continuing.
	...
	int		disposal;		/* Indicates the way in which the graphic is to be treated after being displayed.
...
typedef struct {
	...
	MWUCHAR	*imagebits;		/* [T.J.Park Add] the pointer of the image buffer */
...
#define MAX_LOAD_IMAGE_EX	256		/* Maximum number of images returned by GrLoadImageEx().
                                   This is added to support animated GIF. */

/* image loading structure - returned by GdLoadImageEx
   This is for animated GIF */
typedef struct {
	int count;								/* the count of the image. If fails, this value shall be 0. */
	unsigned int images[MAX_LOAD_IMAGE_EX];	/* the array of image IDs */
} MW_LOAD_IMAGE_EX_RESULT;
