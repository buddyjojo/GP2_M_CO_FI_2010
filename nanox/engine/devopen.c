...
#ifndef xxxALPHABLEND
#define xxxALPHABLEND 0
#endif

#ifndef NOFONTSORCLIPPING
...
MWPIXELVAL	gr_foreground = 0;      /* current foreground color */
MWPIXELVAL	gr_background = 0;      /* current background color */
MWPIXELVAL  gr_xorcolor	  = 0;	    /* added by foryou */
MWBOOL		gr_usebg = FALSE;  	    /* TRUE if background drawn in pixmaps */
int			gr_mode = MWMODE_COPY;	/* drawing mode */
int			gr_alpha = -1;			/* global alpha value, which overrides the alpha channel of each pixel
									   Negative value means this value shall be ignored. */
//int			gr_gradation_startpos = -1;
MWGRADATION gr_gradation = {-1, -1, GRADATION_NONE};
 ...
 */
PSD
GdOpenScreen(int layer)
{
	PSD			psd;
	MWPALENTRY *		stdpal;
	MWSCREENINFO		sinfo;

	psd = scrdev[layer].Open(scrdev + layer, layer);
	if (!psd)
		return NULL;
	...
	/* added by foryou */
	GdSetXorColor(psd, MWRGB(0, 0, 0)); /* added by foryou */
...
/**
 * Set the width/height of the screen at run-time
 */
void
GdSetScreenRes(PSD psd, int width, int height)
{
	psd->SetScreenRes(psd, width, height);
}
...
#if YCBCR
// Translation of ARGB into YCbCr
    MWCOLORVAL r, g, b, y, cb, cr, al;
    int ri, gi, bi;
    ...
    y = ((213 * ri + 715 * gi + 72 * bi)/1000);
    cb = ((-115*ri - 355*gi + 540*bi)/1200 + 128);
    cr = ((500*ri - 434*gi - 55*bi)/1200 + 128);
    ...
    c = ((MWCOLORVAL)( (unsigned char)(y)|
        ((unsigned)((unsigned char)(cb))<<8)|
        //((unsigned long)((unsigned char)(al))<<24)|
        //( 0xff000000ul) |
        ( 0xd0000000ul) |
        ((unsigned long)((unsigned char)(cr))<<16)));
#endif
	/*
	...
	case MWPF_TRUECOLOR1555:
		/* create 16 bit 1/5/5/5 format pixel from RGB colorval*/
...
#ifdef VXWORKS /* VxWorks does not allow pack() directive with no parameter. */
#pragma pack(8)
#else
#pragma pack()
#endif
...
{
#ifndef VXWORKS /* In Vxworks, this function is not supported. This function works only in LINUX. */
#if defined(HAVE_FILEIO)
	int	ifd, i, j;
	FILE *	ofp;
	...
	cx = scrdev[0].xvirtres;
	cy = scrdev[0].yvirtres;
	bpp = scrdev[0].bpp;
	bytespp = (bpp+7)/8;
