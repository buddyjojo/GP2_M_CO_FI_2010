...
#define FREETYPE_FONT_DIR "d:/work/iDCR/App/microwindows/src/fonts/truetype"
#endif
...
GdGetFontList(MWFONTLIST ***fonts, int *numfonts)
{
	...
	sprintf(szDir, "%s\\*", FREETYPE_FONT_DIR);
	...
	*numfonts = idx;
	...
	/* Search again to read the TTF files. */
	hList = FindFirstFile(szDir, &FileData);
	...
	*fonts = list;
	...
	while (!FindNextFile(hList, &FileData))
	{
		...
		if (stricmp(p, ".ttf") == 0)
		{
			/* get full path */
			p = 0;
			pl = strlen(FREETYPE_FONT_DIR)
			   + strlen(FileData.cFileName) * sizeof(char)
			   + 2;
			p = (char*)malloc(pl);
			p = (char*)memset(p, '\0', pl);
			p = (char*)strcat(p, FREETYPE_FONT_DIR);
			p = (char*)strcat(p, "/");
			p = (char*)strcat(p, FileData.cFileName);


			if((ftmp = get_tt_name(p)) != NULL) {
				list[idx]->ttname = ftmp;
				list[idx]->mwname = malloc(strlen(FileData.cFileName) + 1);
				list[idx]->mwname = strcpy(list[idx]->mwname, FileData.cFileName);
