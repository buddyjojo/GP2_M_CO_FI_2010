...
drawarcsegment(SLICE *slice, MWCOORD xp, MWCOORD yp, int drawon)
{
  unsigned long dm = 0, dc = 0;
	...
	case MWELLIPSEFILL:
		...
		{
			if(yp == 0)
			{ /* T.J.Park : The center of the ellipse must be drawn only once. */
				ts_drawrow(slice->psd, slice->x0-xp, slice->x0+xp, slice->y0);
			}
			else
			{
		...
		else
		{
			...
			if(yp == 0)
			{ /* T.J.Park : The center of the ellipse must be drawn only once. */
				drawrow(slice->psd, slice->x0-xp, slice->x0+xp, slice->y0);
			}
			else
			{
...
/* T.J.Park Add : drawarc() function has a problem if the rx or ry is bigger than 1023.
                  (because it multiply rx and ry three times.)
                  One solution may be 64 bit internal calculation, but it can not fix all the problems.
				  So, I put the following code. This clipping is not correct.
				  I don't know how to scissor the arc. */

#define MAX_RADIUS	1023

MWBOOL GdClipArc(MWCOORD *px, MWCOORD *py, MWCOORD *prx, MWCOORD *pry
				, MWCOORD xmin, MWCOORD ymin, MWCOORD xmax, MWCOORD ymax)
{
	MWCOORD x, y, rx, ry;
	MWCOORD rx_sign, ry_sign;

	int outcode; /* 4 bit check value. */
	...
	/* For rx and ry, make it positive and keep the sign for later processing. */
	if(*prx >= 0)
	{
		rx = *prx;
		rx_sign = 1;
	...
	/* Check the outcode of center */
	switch(outcode)
	{
		case 9: /* 1001 : Center is outside of top-left corner */
			if(x + rx < xmin
			|| y + ry < ymin)
				return FALSE;
			break;
		case 8: /* 1000 : Center is outside of upper bound. */
			if(y + ry < ymin)
				return FALSE;
			break;
		case 10: /* 1010 : Center is outside of top-right corner */
			if(x - rx > xmax
			|| y + ry < ymin)
				return FALSE;
			break;
		case 1: /* 0001 : Center is outside of left corner */
			if(x + rx < xmin)
				return FALSE;
			break;
		case 0: /* 0000 : Center is on the plain */
			break;
		case 2: /* 0010 : Center is outside of right corner */
			if(x - rx > xmax)
				return FALSE;
			break;
		case 5: /* 0101 : Center is outside of bottom-left corner */
			if(x + rx < xmin
			|| y - ry > ymax)
				return FALSE;
			break;
		case 4: /* 0100 : Center is outside of bottom corner */
			if(y - ry > ymax)
				return FALSE;
			break;
		case 6: /* 0110 : Center is outside of bottom-right corner */
			if(x - rx > xmax
			|| y - ry > ymax)
				return FALSE;
			break;
	...
	if(x < MIN_MWCOORD)
		x = MIN_MWCOORD;
	else if(x > MAX_MWCOORD)
		x = MAX_MWCOORD;
	if(y < MIN_MWCOORD)
		y = MIN_MWCOORD;
	else if(y > MAX_MWCOORD)
		y = MAX_MWCOORD;
